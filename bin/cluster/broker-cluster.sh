...
currentScript="broker-cluster.sh"
...
jsonCluster=$(find $commandPath -name broker-cluster.json | head -n 1)
brokerCluster=$(cat $jsonCluster)
rerun=$(echo sh $(find $commandPath -name broker-cluster.sh | head -n 1))
tab='  '
...
firstSettings() {
	...
	id=0
	port=$(sh $(find $commandPath -name broker-all-settings.sh | head -n 1) $serverPath broker-port)
	dir=$(sh $(find $commandPath -name broker-all-settings.sh | head -n 1) $serverPath log-dir)
...
do
	...
	then
		...
		gnome-terminal -- bash -c "sh $(find $path -name broker-cluster-assembler.sh | grep -v Trash | head -n 1); exec bash"
